...
DAPP_DIR = $(CURDIR)/radial
SRC_DIR = $(CURDIR)/src
SRCS = $(addprefix $(SRC_DIR)/, radial.md lemmas.k.md storage.k.md prelude.smt2.md)
...
$(DOC_DIR)/radial.html: $(SRCS)
	$(info Generating html documentation: $@)
	mkdir -p $(DOC_DIR)
	$(KLAB_FLAGS) klab report > $@

doc: $(DOC_DIR)/radial.html

doc-publish: $(DOC_DIR)/radial.html
